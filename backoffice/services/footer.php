...
use App\Core\CRUD\Footer;
use App\Core\Box\Tools;

require_once '../App/Core/CRUD/Footer.php';
...
function update()
{
    ...
    $serviceManager = new Footer();
    $service_data[0] = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT);
    $service_data[1] = filter_input(INPUT_POST, 'direccion', FILTER_SANITIZE_STRING);
    $service_data[2] = filter_input(INPUT_POST, 'telefono', FILTER_SANITIZE_STRING);
    $service_data[3] = filter_input(INPUT_POST, 'correo', FILTER_SANITIZE_STRING);

    $qres = $serviceManager->updateFooterById($service_data);
